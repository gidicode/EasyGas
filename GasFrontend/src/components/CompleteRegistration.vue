...
<template>
  <div class="grid grid-col-2 place-items-center">
    <div class="mt-10" v-if="userStatusVendor">      
      <div v-if="goNext">
        ...
        :state="state"
        :errors="errors"
        class="mt-4 grid grid-cols-1 place-items-center" />  
      </div>
      <div v-else>
        <p class="text-base font-semibold text-[#3D1174]">
    ...
    <div class="mt-10" v-else>      
      ...
      <user-complete-reg 
        :state="state"
        :errors="errors"
        class="mt-2 grid grid-cols-1 place-items-center" />
    </div>    

    <button 
      v-if="userStatusVendor"
      @click="changeGoNext"
    ...
    <div class="mt-5" v-if="goNext">
      ...
      <button
        v-else
        class="rounded-xl mb-2 bg-[#8157B6] p-2 w-36 h-10 text-white text-base text-center font-medium"
      >
        Submit
      </button>      
    </div>
...
const userStatusVendor = ref(null);
const UserRegComplete = ref(null);
const VendorRegComplete = ref(false);
const { loading } = storeToRefs(store);

onMounted(() => {});
watch(loading, (value) => {
  if (value == false) {
    console.log("jeeee", value);
    UpdateUser();
  }
});
...
function UpdateUser() {
  ...
  console.log(currentUser.value, "current user value check");
  userStatusVendor.value = currentUser.value.vendor;
  UserRegComplete.value = currentUser.value.reg_complete;
}

//defineProps(["userStatusVendor"]);
//const userStatusVendor = ref(props.userStatusVendor)

const goNext = ref(false);
const changeGoNext = () => (goNext.value = !goNext.value);
</script>
